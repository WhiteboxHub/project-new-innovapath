...
function buildUrl(endpoint) {
  const baseUrl = (process.env.NEXT_PUBLIC_API_URL || "https://whitebox-learning.com").replace(/\/$/, "");
...
export async function apiFetch(endpoint, options = {}) {
  ...
  if (endpointStr.startsWith("http")) {
    ...
    // It's a relative path. Check if it already has 'api/' prefix.
    // If our backend routes are /api/..., we should ensure we match that.
    ...
    if (endpointStr.startsWith("api/")) {
      ...
      // Only prepend api/ if missing, strict check
      if (!baseUrl.endsWith("/api")) {
        endpointStr = `api/${endpointStr}`;
      }
    }
  }
...
export const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || "https://whitebox-learning.com";
export default api;
